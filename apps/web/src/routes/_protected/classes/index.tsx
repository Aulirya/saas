...
import { SchoolClassWithSubjectsAndLessons } from "shared";
...
function ClassesPage() {
  ...
  const { data: levels = [], isLoading: isLoadingLevels } = useClassesLevels();
...
function ClassCard({
  ...
  return (
    <SelectableCard
      ...
      description={
        <span>
          {classData.school} • {classData.level} selected: {isSelected}
    ...
    return (
      <div className="space-y-4 py-4">
        <p className="text-sm text-muted-foreground">
          Sélectionnez une classe pour afficher son résumé et ses statistiques.
        </p>
      </div>
    );
  }
