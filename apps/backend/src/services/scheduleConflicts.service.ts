  ...
  try {
    ...
    if (subjectIds.length > 0) {
      for (const subjectId of subjectIds) {
        ...
        if (subjects.length > 0) {
          subjectNamesMap.set(subjects[0].id.toString(), subjects[0].name);
        }
      }
    }
    ...
    for (const newSlot of recurringSchedule) {
      for (const otherCp of allCourseProgress) {
        ...
        const subjectName =
          subjectNamesMap.get(otherCp.subject_id.toString()) || "cours inconnu";
          ...
          if (
            ...
            conflicts.push({
              ...
              message: `Conflit avec le cours "${subjectName}" le ${getDayName(
                newSlot.day_of_week,
              )} de ${existingSlot.start_hour}h à ${existingSlot.end_hour}h`,
              slot: newSlot,
              conflicting_course_progress_id: otherCp.id.toString(),
            });
          }
        }
      }
