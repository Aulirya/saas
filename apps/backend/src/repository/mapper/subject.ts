import type { Subject, SUBJECT_CATEGORIES } from "shared";
...
export namespace SubjectMapper {
  ...
  export function fromModel(
    ...
    return {
      id: getValue(modelWithNested, "id")?.toString() ?? model.id.toString(),
      name: getValue(modelWithNested, "name") ?? model.name,
      description:
        getValue(modelWithNested, "description") ?? model.description ?? "",
