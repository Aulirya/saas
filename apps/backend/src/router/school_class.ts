...
export const listSchools = base.handler(
  async ({ context }): Promise<string[]> => {
    ...
    const schools = result[0]
      .map((row) => row.school)
      .filter(
        (school): school is string => school !== null && school !== undefined,
      );
...
export const listLevels = base.handler(
  async ({ context }): Promise<string[]> => {
    ...
    const levels = result[0]
      .map((row) => row.level)
      .filter(
        (level): level is string => level !== null && level !== undefined,
      );
...
export const getSchoolClassWithSubjects = base
  .input(z.object({ id: z.string() }))
  .handler(
    async ({ input, context }): Promise<SchoolClassWithSubjectsAndLessons> => {
      const userId = new RecordId("users", context.user_id);
      const classId = parseRecordId(input.id, "classes");
    ...
    try {
      ...
      if (input.subjects && input.subjects.length > 0) {
        ...
        for (const subjectId of input.subjects) {
          const subjectRecordId = parseRecordId(subjectId, "subjects");
          await context.db
  ...
  .handler(async ({ input, context }): Promise<SchoolClass> => {
    ...
    try {
      ...
      if (Object.keys(updateData).length > 0) {
        await context.db.update<SchoolClassModel>(classId).merge(updateData);
      }
