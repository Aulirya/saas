import type { CourseProgressWithLessons } from "shared";
...
export function transformCourseProgressToProgram(
  ...
  if (
    ...
    completedHours = courseProgress.lesson_progress
      ...
      .reduce((sum, lp) => {
        ...
        const lesson = subjectLessons.find((l) => l.id === lp.lesson_id);
  ...
  if (
    ...
    nextLessons = courseProgress.lesson_progress
      .filter((lp) => lp.status === "scheduled" || lp.status === "not_started")
      .map((lp) => {
        const lesson = subjectLessons.find((l) => l.id === lp.lesson_id);
